  ...
  return (
    <div className="home-login-join">
      <div className={`input-wrap ${validationFail && "validation"}`}>
        ...
        <input id="user_id" className={`input ${validationFail && "validation"}`} type="text" placeholder="아이디를 입력해주세요."
          value={userId} onChange={ (event) => {setUserId(event.target.value); setValidationFail(false);} } />
        
        <input id="user_pw" className={`input ${validationFail && "validation"}`} type="password" placeholder="비밀번호를 입력해주세요."
